...
class Trainer:
    def __init__(self, is_unittest=False):
        ...
        self.clip_len = -1
        # self.clip_len = 15 * 90
        hgd_train = HgdHandcraft('dynamic', Path.home() / 'MeetingHands', True, (512, 512), clip_len=self.clip_len)
        hgd_val = HgdHandcraft('dynamic', Path.home() / 'MeetingHands', False, (512, 512), clip_len=self.clip_len)
        ...
        hgd = HgdHandcraft('dynamic', Path.home() / 'MeetingHands', True, (512, 512), clip_len=self.clip_len)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            model_path = self.model_folder / Path('lstm_model_epoch_' + str(epoch) + '_valacc_' + str(round(acc_valed, 2)) + '.pt')
            WSWA2_test_ap = 0
            WSWA3_test_ap = 0
            WSWA4_test_ap = 0
            WSWA5_test_ap = 0
    ...
    def val(self, model):
        ...
        for ges_data in self.val_loader:
            ...
            _, h, c, class_out = model(features, h0, c0)
            target = ges_data[HG.GESTURE_LABEL]
            target = target.to(model.device, dtype=torch.long)
            target = target.permute(1, 0)
            target = target.reshape((-1))
            np_out = class_out.cpu().detach().numpy()
            r_hands = []
            for i in range(np_out.shape[0]):
                _ = np.argmax(np_out[i])
                r_hands.append(_)
            target = target.cpu().detach().numpy()
            r_hands = np.asarray(r_hands)
